...
export function AdminWithdrawsClient() {
  ...
  async function load() {
    setLoading(true);
    const r = await fetch("/api/admin/withdraws?status=PENDING");
  ...
  async function act(id: string, type: "approve" | "reject") {
    ...
    const res = await fetch(`/api/admin/withdraws/${id}/${type}`, {
      method: "POST",
  ...
  return (
    <div className="grid gap-4">
      <div className="card p-6">
        <h1 className="text-xl font-semibold">Withdraw Requests (PENDING)</h1>
        <p className="mt-1 text-slate-600 text-sm">Approve akan mengurangi saldo user + membuat ledger. Transfer bank dilakukan manual.</p>
          ...
          <table className="min-w-full text-sm">
            <thead className="text-slate-500">
              <tr className="border-b">
                ...
                <th className="py-2 text-left">Rekening Tujuan</th>
            ...
            <tbody>
              {rows.map((x) => (
                <tr key={x.id} className="border-b last:border-b-0">
                  ...
                  <td className="py-2">
                    <div className="font-medium">{x.bankName}</div>
                    <div className="text-slate-600">{x.accountNo} a.n. {x.accountName}</div>
                  </td>
